...
# =====================================================================@005918=
# 1. podnaloga
# Dodajte metodo `odrezi(self, n)`, ki odstrani vsa vozlišča, ki ležijo
# na globini (oz. nivojih) večji od `n`. Zgled:
#
#     >>> d = Drevo(5,
...
class Drevo:
    ...
    #     def globina(drev, drevo, nivo):
    #         if nivo < n:
    #             if not drevo.prazno:
    #                 drev = Drevo(drevo.vsebina)
    #                 # drev2 = drev
    #                 # l = self.levo
    #                 # d = self.desno
    #                 # nivo += 1
    #                 globina(drev.levo, drevo.levo, nivo + 1)
    #                 globina(drev.desno, drevo.desno, nivo + 1)

    #         print(drev)

    #     globina(drevo2, self, 0)
    #     # globina(self, n)
    #     # self.prazno = drevo2.levo
    #     self.vsebina = drevo2.vsebina
    #     self.levo = drevo2.levo
    #     self.desno = drevo2.desno

    def odrezi(self, n):
        if self.prazno:
            pass  # Če je drevo prazno, ne naredimo ničesar.
        elif n < 1:
            # Če je nivo padel pod ena, drevo izpraznimo.
            self.prazno = True
            # Poleg tega pobrišimo atribute, ki jih prazno drevo ne rabi.
            del self.vsebina
            del self.levo
            del self.desno
        else:
...
# =====================================================================@005919=
# 2. podnaloga
# Sestavite metodo `potroji(self)`, ki pod vsakim vozliščem doda še dve
# njegovi kopiji. Levo poddrevo prestavi levo od leve kopije, desno
# poddrevo pa desno od desne kopije. Zgled:
#
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1OTE4LCJ1c2VyIjo2OTI3fQ:1pDj8q:AZLm_5VuSmOQc_QefVAXW2fL96VCVwLqTqdSX-1c3VQ"
        try:
            test_data = [
                (
                    [
                ...
                (
                    ...
                    ],
                    {
                        "d": Drevo(
                            5,
                            levo=Drevo(4, desno=Drevo(2)),
                            desno=Drevo(3, levo=Drevo(4), desno=Drevo(4)),
                        )
                    },
                ),
            ]
    ...
    if Check.part():
        Check.current_part[
            "token"
        ] = "eyJwYXJ0Ijo1OTE5LCJ1c2VyIjo2OTI3fQ:1pDj8q:ksn0u1WjOLnKUVJ3DBtNH6TlVF4Fem6vqTQw4T9ul8I"
        try:
            test_data = [
                (
                    ["d = Drevo(5, levo=Drevo(3), desno=Drevo(2))", "d.potroji()"],
                    {
                        "d": Drevo(
                            5,
                            levo=Drevo(
                                5,
                                levo=Drevo(3, levo=Drevo(3), desno=Drevo(3)),
                                desno=Drevo(),
                            ),
                            desno=Drevo(
                                5,
                                levo=Drevo(),
                                desno=Drevo(2, levo=Drevo(2), desno=Drevo(2)),
                            ),
                        )
                    },
                ),
                ...
                (
                    ["d = Drevo(1, levo=Drevo(2), desno=Drevo(3))", "d.potroji()"],
                    {
                        "d": Drevo(
                            1,
                            levo=Drevo(1, levo=Drevo(2, levo=Drevo(2), desno=Drevo(2))),
                            desno=Drevo(
                                1, desno=Drevo(3, levo=Drevo(3), desno=Drevo(3))
                            ),
                        )
                    },
                ),
